...
class Router {
    ...
    handleRoute() {
        ...
        // Check if this is an anchor/fragment within current section (not a section route)
        if (!this.sections.includes(section)) {
            // If we're already on the current section and the hash is a fragment, just allow it
            if (this.currentSection && document.getElementById(section)) {
                // This is an anchor within the current page, let browser handle it
                return;
            }
            
            // Otherwise, this is an invalid section, default to home
            section = 'home';
            window.location.hash = '#home';
        }
     ...
     */
    initSlideshow() {
        // Use the Slideshow class defined in main.js
